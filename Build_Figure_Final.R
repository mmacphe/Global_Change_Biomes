#Start by downloading the World Wildlife Fund terrestrial ecoregions (https://www.worldwildlife.org/publications/terrestrial-ecoregions-of-the-world)
#Then, download the CanESM5 current and future climate from Worldclim (https://www.worldclim.org/data/cmip6/cmip6climate.html).
rm(list=ls())
...
#Then, read in biome 'mask' layer (e.g., Biome1)
Biome1<-readOGR(dsn=paste0('./GlobalBiomes_Shapefiles/BIOME1.shp'))

#read in environmental data layers from Worldclim
...
#Change Biome numbers to their names. Note: Follow the order below.
...
a<-ggplot(Temperate, aes(x=temp,y=precip, color=Biome)) +
  ...
  xlim(-50,30) +
  ylim(0,800) +
  labs(y="Precipitation (mm)", x="Temperature (C)") +
  theme_bw() + 
...
png(file='./Output Files/TemperateBiomes.png',width=6.5,height=5.5,units="in",res=500)
ggarrange(a,labels=c("A"))
dev.off()

png(file='./Output Files/TropicalBiomes.png',width=6.5,height=5.5,units="in",res=500)
ggarrange(b,labels=c("B"))
dev.off()

png(file='./Output Files/TemperateBiomes2.png',width=6.5,height=5.5,units="in",res=500)
ggarrange(c,labels=c("C"))
dev.off()

png(file='./Output Files/TropicalBiomes2.png',width=6.5,height=5.5,units="in",res=500)
ggarrange(d,labels=c("D"))
dev.off()

#These were arranged and annotated further using Microsoft PowerPoint.